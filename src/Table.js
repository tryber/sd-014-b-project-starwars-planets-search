...
const columFilter = ['population', 'orbital_period',
...
const Table = () => {
  ...
  return (
    <>
      ...
      >
        {columFilter.map((item, index) => <option key={ index }>{item}</option>)}
      </select>
      <select
      ...
      >
        {comparisonFilter.map((item, index) => <option key={ index }>{item}</option>)}
      </select>
      <input
        onChange={ ({ target }) => {
          setData(results);
